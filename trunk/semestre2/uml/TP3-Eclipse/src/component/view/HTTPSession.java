...
public class HTTPSession extends JPanel{
	...
	public HTTPSession(JTabbedPane container){
		...
		firstButton.setToolTipText("Première Page.");
		...
		lastButton.setToolTipText("Dernière Page.");
	...
	public void setDefaultPage(){	
		...
		if(!file.exists()){
			try { 
				...
				JOptionPane.showMessageDialog(null, "La page par defaut n'a pu être atteinte.", "RAMClient",
	...
	private void hyperlinkAction(HyperlinkEvent e) {
		HyperlinkEvent.EventType type = e.getEventType();
	      if (type == HyperlinkEvent.EventType.ACTIVATED){
	    	  if(e.getURL()!=null){
	    		  statusBar.setText("Accès à "+e.getURL().toExternalForm()+"...");
	...
	public void printAction() {
		if(editor!=null){
			try {
					...
					JOptionPane.showMessageDialog(null, "Un problème a été détecté lors du lancement de l'impression.\n" +
							"Vérifiez que votre imprimante est bien connectée", "RAMClient", JOptionPane.ERROR_MESSAGE);
					e.printStackTrace();
			}
		}
	}
		...
		if(current!=-1){
			if(Desktop.isDesktopSupported() && Desktop.getDesktop().isSupported(Desktop.Action.BROWSE) ) {
				try {
					...
					JOptionPane.showMessageDialog(null, "Un problème est survenu lors du lancement de votre navigateur." +
							"\nVerifiez qu'il est correctement installé","RAMClient" , JOptionPane.ERROR_MESSAGE);
				}
			}else
				JOptionPane.showMessageDialog(null, "Votre Système ne supporte pas ce type d'opération.","RAMClient" ,
							JOptionPane.WARNING_MESSAGE);
		}else
			JOptionPane.showMessageDialog(this, "Cette page ne peut être ouverte sur un autre navigateur","RAMClient" ,
