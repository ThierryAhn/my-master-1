 ...
 * 
 */
@WebService(endpointInterface = "discovery.service.CityManager", serviceName = "CityManager")
public class CityManagerWebService extends AvailableService implements CityManager{
	 ...
	 */
	public CityManagerWebService(Repository repository) {
	...
	/**
	 * Return a list of cities with the name in parameter by searching in all repositories available.
	 ...
	 */
	public List<City> searchFor(String cityName) throws MalformedURLException, ServiceNotFound, Exception{
		
		List<City> allCities = new LinkedList<City>();
		
		if(repository != null){
			for(String serviceName : repository.listAllServices()) {
				ServiceDescription serviceDescription = repository.searchFor(serviceName);
				URL wsdlURL = new URL(serviceDescription.getUrl()+"?wsdl");
				
				Service service = Service.create(wsdlURL, serviceDescription.getServiceName());
				CityManager cityManager = service.getPort(serviceDescription.getPortName(), CityManager.class);
				
				allCities.addAll(cityManager.searchForLocalCity(cityName));
			}
		}
		
		if(allCities.isEmpty()) throw new CityNotFound("Ville non trouvee");
	 ...
	 */
	public List<City> searchForLocalCity(String cityName) {
		List<City> listCities = new LinkedList<City>();
		for(City city : cities){
